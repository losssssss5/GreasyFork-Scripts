        ...
        <fieldset id="${def.id.field}" style="display:block">
          ...
          <ul class="${def.class.main}">
            ...
            <li id="${def.id.fontFace}">
              <div style="margin:0;padding:0">글꼴변경（기본\uff1a켜기）</div>
          ...
          try {
            ...
            qS(`#${def.id.fonttooltip}`, def.const.configIf)?.addEventListener("dblclick", async () => {
              ...
              let frDialog = new FrDialogBox({
                ...
                neutralButtonText: "취 소",
                messageText: `<p style="color:#555555;font-size:14px!important">다음 텍스트 필드에는 미리 정해진 형식의 사용자 지정 글꼴을 추가할 수 있습니다. 팁을 사용하거나 샘플에 따라 필드를 채우면 입력 오류가 자동으로 필터링됩니다. 다음 항목과 비교『<a href="${def.const.gfHost}#fontlist" title="기본 제공 글꼴 목록 보기" target="_blank">기본 제공 글꼴 목록</a>』중복 글꼴은 자동으로 제외됩니다.。【기능 도움말\uff1a<span id="${def.const.seed}_addTools" title="클릭하여 도구 열기" style="color:crimson;cursor:pointer">글꼴 추가 보조 도구</span>】</p><p><textarea id="${def.const.seed}_custom_Fontlist" style="box-sizing:border-box;margin:0!important;padding:5px!important;max-width:388px!important;min-width:388px!important;min-height:160px!important;outline:none!important;border:1px solid #999999;border-radius:6px;white-space:pre;font:normal 400 14px/150% monospace,${INITIAL_VALUES.fontSelect},system-ui,-apple-system,BlinkMacSystemFont,sans-serif!important;resize:vertical;scrollbar-width:thin;overscroll-behavior:contain" placeholder='다음과 같이 한 줄당 하나의 글꼴 이름 데이터 세트가 있는 글꼴 테이블의 사용자 지정 형식 샘플이 있습니다.\uff1a\r\n{ "ch":"중국어 글꼴 이름 1","en":"EN Fontname 1" }\u21b2\r\n{ "ch":"중국어 글꼴 이름 2","en":"EN Fontname 2","ps":"Post-Script Name" }\u21b2\r\n\r\n（注一\uff1a“ps:”이 항목은 PostScript 글꼴의 이름입니다，선택 사항）\r\n（注二\uff1a\u21b2줄 바꿈，입력(Enter)하고 차량으로 돌아가세요.）'>${received_Fontlist}</textarea></p><p style="display:block;margin:-5px 0 0 -7px!important;height:max-content;color:#dc143c;font-size:14px!important">（페이지 로딩이 느려지지 않도록 사용자 정의 글꼴을 너무 많이 추가하지 마세요.）</p>`,
